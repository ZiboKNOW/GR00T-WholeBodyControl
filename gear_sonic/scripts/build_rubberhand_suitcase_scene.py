#!/usr/bin/env python3
"""Build MuJoCo sim scene for unitree_g1_sonic_no_hand_wo_wrist sim2sim.

Aligns with git scene_suitcase_43dof (Inspire DFQ body + damping + scene wrapper),
except:
  - Inspire hands removed; rubber_hand visual + Isaac eef_box wrist contacts
...
# L-shaped wrist contacts — same geometry as Isaac training
# (g1_29dof_rubberhand-feet_sphere-eef_box-body_capsule /
#  HDMI g1_29dof_nohand-eef_L-box.xml). Invisible in sim for rendering only.
_EEF_BOX_A = (
    'type="box" size="0.03 0.04 0.002" pos="0.13 0 0" '
    'rgba="0 0 0 0" contype="1" conaffinity="1" group="3"'
)
_EEF_BOX_B = (
    'type="box" size="0.002 0.04 0.03" pos="0.10 0 -0.03" '
    'rgba="0 0 0 0" contype="1" conaffinity="1" group="3"'
)
...
# Match HDMI Isaac render_vla / move_suitcase_sonic tiled_camera on d435_link/front_cam.
# See HDMI/active_adaptation/envs/locomotion.py (D435 intrinsics) and
# active_adaptation/envs/camera_configs.py CameraPresets.g1_front_camera().
ISAAC_D435_LINK_POS = "0.0576235 0.01753 0.41987"
ISAAC_D435_LINK_QUAT = "0.91496 0 0.403545 0"
ISAAC_HEAD_CAMERA_XML = (
    '<camera name="head_camera" pos="0.01 0 0" quat="0.5 0.5 -0.5 -0.5" '
    'focalpixel="487.8023681640625 487.8023681640625" '
    'principalpixel="325.7099609375 233.83817545572917" '
    'resolution="640 480" sensorsize="0.01 0.01"/>'
)
...
def _align_isaac_ego_camera(xml: str) -> str:
    """Align d435_link mount + head_camera intrinsics/extrinsics with Isaac Sim."""
    xml = re.sub(
        r'<body name="d435_link" pos="[^"]*" quat="[^"]*">',
        f'<body name="d435_link" pos="{ISAAC_D435_LINK_POS}" quat="{ISAAC_D435_LINK_QUAT}">',
        xml,
        count=1,
    )
    xml = re.sub(
        r'<camera name="head_camera"[^>]*/>',
        ISAAC_HEAD_CAMERA_XML,
        xml,
        count=1,
    )
...
def build_robot_xml() -> str:
    ...
    xml = _align_isaac_ego_camera(xml)
    return xml
