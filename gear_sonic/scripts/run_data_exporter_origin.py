...
from gear_sonic.data.features_sonic_vla import (
    ...
    get_g1_robot_model,
    get_global_view_camera_features,
    get_global_view_camera_modality_config,
...
class SonicDataExporterConfig:
    ...
    """Record the fixed overview (god's-eye) camera in addition to ego_view (head camera)."""
...
def main(config: SonicDataExporterConfig):
    ...
    if config.record_global_view:
        ...
        dataset_features.update(get_global_view_camera_features())
        global_modality = get_global_view_camera_modality_config()
        for key, value in global_modality.items():
            if key in modality_config:
                modality_config[key].update(value)
            else:
                modality_config[key] = value
    ...
    data_exporter = Gr00tDataExporter.create(
        ...
        script_config={
            **robot_config,
            ...
            "record_global_view": config.record_global_view,
        },
    )
